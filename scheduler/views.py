...
def home_view(request):
    ...
    time_list = [single_date.strftime("%H:%M") for single_date in daterange(start_date, end_date)]
...
def load_csvs():
    ...
    with open(locations_file) as csv_locations:
        ...
        for row in csvreader:
            ...
            if created:
                print("{} - {} inserted to the database".format(row[1], row[2]))
    ...
    with open(work_orders_file) as csv_work:
        ...
        for row in csvreader:
            ...
            converted_date_time = datetime.strftime(original_datetime, "%Y-%m-%d %H:%M") 
            ...
            if created:
                print("Work order created for {} at {}".format(technician.technician_name, row[3]))
